...
async function main() {
  ...
  const licenseServerUrl =
    configuredUrl || DEFAULT_LICENSE_SERVER_URL;
  ...
  try {
    const notesValue =
      notes ||
      (await rl.question("更新说明（直接回车跳过）: ")).trim();
    const confirm =
      (await rl.question(
        `确认把版本 ${version} 注册到授权中心 ${licenseServerUrl} ？(y/N) `,
      )).trim().toLowerCase();
    ...
    const response = await fetch(
      `${licenseServerUrl.replace(/\/+$/, "")}/api/v1/releases`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok || !body?.ok) {
      throw new Error(
        `注册失败（HTTP ${response.status}）：${body?.error ?? JSON.stringify(body)}`,
      );
    }
