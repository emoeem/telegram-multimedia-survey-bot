...
- **报告打包直发私人频道**：归档/重发时若答卷含用户上传图片，PDF + 图片用 fflate 打包为单个 `report-{id}.zip` 发到私人频道（连带问卷/答卷/用户/时间信息与 hashtag）；超过 45MB 自动降级为 PDF + 最多 6 张图片分别发送。答卷详情「导出到私人频道（PDF+图片打包）」按钮触发
- **后台响应式**：编辑器（草稿）桌面端左右分栏——左侧问卷设置+题目列表，右侧粘性实时预览（自动跟随编辑状态）；移动端保持弹窗预览；列表/表格页沿用现有自适应布局与横向滚动
- **答卷批量导出 + 内嵌预览 + 分享链接（2026-08-23）**：答卷列表勾选已完成答卷 → 一键批量入队发送到私人频道（每份独立 zip）；桌面端答卷列表右侧内嵌 Web 报告预览（可切换模板，iframe）；每份答卷可复制 30 天有效的分享链接（签名 token）
- **提升批 1-7（2026-08-23）**：
  1. 系统设置真正生效：上传单张大小 / 答卷图片总量 / 临时媒体 TTL / PDF 大小上限均读取后台设置
  2. 公开问卷列表升级：支持搜索（`?q=`）、封面图（`cover_media_id` 直链）、主题透传
  3. 审计日志页：`/admin/audit` 完整审计列表（时间/执行者/操作/对象/变更摘要，按对象筛选分页）
  4. 编辑器撤销/重做 + 自动保存：Ctrl/Cmd+Z / Ctrl+Shift+Z / Ctrl+Y，2 秒空闲自动保存；undo/redo 基于「快照 diff 重建操作队列」，保存后基线重置
  5. PWA：manifest + service worker（生产 HTTPS 注册），可添加到主屏幕、离线壳
  6. 模板编辑器：块拖拽排序（dnd-kit）+ 字体/主色/强调色可视化控件（生成 CSS 变量）+ 额外 CSS 输入框
  7. 批量导出进度：导出后轮询 `export-status` 显示「已发送 X/Y」，完成后汇总成功/失败
- **GitHub CI（2026-08-23）**：`.github/workflows/ci.yml`——push/PR 自动跑 typecheck / 单测 / lint / admin 构建 / PDF 解析器回归 + Playwright 视觉回归（含 CJK 字体）
