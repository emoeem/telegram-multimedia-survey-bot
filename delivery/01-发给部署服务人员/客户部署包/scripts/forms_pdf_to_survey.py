...
except ModuleNotFoundError:  # Friendly message for direct, first-time use.
...
class UnlabeledChoiceDetector:
    ...
    def detect(
        ...
        if (
            ...
            and left_spread <= 10
            and gap_spread <= 8
            and height_spread <= 12
        ):
            return True, 0.92, "uniform_unlabeled_choice_blocks"
        # Microsoft Forms often lays out unlabeled options with a larger,
        # almost uniform row gap (around 30-40pt). They are indented from the
        # title and have a matching left edge, which distinguishes them from
        # ordinary wrapped question text.
        title_box = question.title_bbox
        title_indent = (
            min(lefts) - title_box[0]
        ...
        if (
            ...
            and gap_spread <= 13
            and height_spread <= 12
            and 10 <= title_indent <= 96
        ):
            return True, 0.84, "uniform_indented_unlabeled_choice_blocks"
        if max(gaps) <= 28 and left_spread <= 18 and gap_spread <= 14:
...
def _finalize_question(question: QuestionDraft) -> None:
    ...
    # Some Forms PDFs place the required marker on its own wrapped line.
    # Keep the required flag, but never expose that marker as question text.
    question.title = _clean_required_title_text(question.title)
...
def repair_import_choice_questions(
    questions: list[dict[str, Any]],
) -> list[str]:
    warnings: list[str] = []
    choice_types = {"single", "multiple", "yes_no", "rating"}
    ...
    for index, question in enumerate(questions):
        ...
        title = normalize(str(question.get("title") or "未命名题目"))
        if len(options) == 1:
            option = options[0]
            text = str(
                option.get("text")
            ...
            if (
                len(parts) == 2
                and all(len(part) <= 24 for part in parts)
                and all(
                    re.search(r"[，,。！？!?；;：:]", part) is None
                    for part in parts
                )
            ):
                ...
                question["options"] = [
                    {
                        **option,
                        "id": f"{question['id']}_o{part_index}",
                        "label": str(part_index),
                        "text": part,
                        "value": part,
                        "order": part_index,
                        "media": option_media if part_index == 1 else [],
                    }
                    for part_index, part in enumerate(parts, start=1)
                ]
                question.setdefault("warnings", []).append(
                    "split_merged_short_options"
                )
                warnings.append(
                    f"第 {index + 1} 题“{title}”拆分了两个被换行合并的选项"
                )
                continue
        ...
        question["options"] = []
        question.setdefault("warnings", []).append(
            "choice_converted_to_text"
        )
        warnings.append(
            f"第 {index + 1} 题“{title}”可识别选项不足两个，已转为文本题"
        )
...
def build_output(
    ...
    import_report = {
        ...
        "choice_repair_warnings": choice_repair_warnings,
    }
    ...
    return {
        ...
        "survey": {
            ...
            "metadata": {
                ...
                "warnings": (
                    parser_warnings
                    + page_mapping_warnings
                    + choice_repair_warnings
                ),
            },
        },
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="将 Microsoft Forms PDF 转为可上传机器人导入的 survey.json。"
    )
    parser.add_argument("input", nargs="?", help="PDF 文件路径；省略时会提示输入")
    parser.add_argument("-o", "--output", help="输出目录；默认在 PDF 同目录创建“文件名-问卷导入”文件夹")
    parser.add_argument("--title", default=None, help="覆盖问卷标题")
    args = parser.parse_args()

    if fitz is None:
        print(
            "缺少 PDF 处理组件 PyMuPDF。\n"
            "请运行一键脚本：Windows 双击 03-PDF转问卷JSON.cmd；Linux 运行 bash scripts/pdf-to-survey-easy.sh。\n"
            "或手动安装：python -m pip install pymupdf",
            file=sys.stderr,
        )
        return 3
    ...
    if not input_path:
        try:
            input_path = input("请把 PDF 文件拖进此窗口后按回车：").strip().strip('"')
        except EOFError:
            input_path = ""
    if not input_path:
        print("未选择 PDF 文件。示例：python scripts/forms_pdf_to_survey.py \"问卷.pdf\"", file=sys.stderr)
        return 2
    ...
    if not pdf_path.is_file() or pdf_path.suffix.lower() != ".pdf":
        print(f"不是有效的 PDF 文件：{pdf_path}", file=sys.stderr)
        return 2

    output_dir = (
        Path(args.output).expanduser()
        if args.output
        else pdf_path.parent / f"{pdf_path.stem}-问卷导入"
    )
    ...
    low_confidence = report["low_confidence_questions"]
    print("\n转换完成。")
    print(f"识别到：{report['pages']} 页、{report['detected_questions']} 题、{report['detected_options']} 个选项。")
    print(f"请上传给机器人的是：\n{survey_path}")
    print("不要上传 document.json；它只是排错明细。")
    if low_confidence:
        numbers = [str(item["source_number"]) for item in low_confidence if item.get("source_number")]
        preview = "、".join(numbers[:20])
        more = "……" if len(numbers) > 20 else ""
        print(f"建议保存前重点检查 {len(low_confidence)} 道低置信度题：{preview}{more}")
    print(f"完整识别报告：{output_dir / 'import-report.json'}")
    return 0
