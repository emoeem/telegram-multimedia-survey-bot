...
from forms_pdf_to_survey import (  # noqa: E402
    QuestionDraft,
    UnlabeledChoiceDetector,
    _finalize_question,
    repair_import_choice_questions,
    run_pipeline,
)
...
class SemanticPdfParserRegression(unittest.TestCase):
    ...
    def test_import_choice_repairs_are_conservative(self) -> None:
        questions = [
            {
                "id": "q_1",
                "type": "single",
        ...
        warnings = repair_import_choice_questions(questions)

        self.assertEqual(
            [option["text"] for option in questions[0]["options"]],
            ["是", "还没有"],
        )
        self.assertEqual(questions[1]["type"], "text")
        self.assertEqual(questions[1]["options"], [])
        self.assertIn("PDF 识别到的原选项内容", questions[1]["description"])
        self.assertEqual(len(warnings), 2)

    def test_detects_widely_spaced_indented_unlabeled_options(self) -> None:
        question = QuestionDraft(
            ...
            source_page_start=1,
            source_page_end=1,
            bbox=[100, 100, 260, 118],
            title_bbox=[100, 100, 260, 118],
        )
        lines = [
            {"text": "选项一", "bbox": [150, 145, 210, 156], "page_number": 1},
            {"text": "选项二", "bbox": [150, 182, 210, 193], "page_number": 1},
            {"text": "其他", "bbox": [150, 220, 190, 231], "page_number": 1},
        ]
        ok, confidence, reason = UnlabeledChoiceDetector().detect(lines, question, None)
        self.assertTrue(ok)
        self.assertGreaterEqual(confidence, 0.8)
        self.assertIn(
            reason,
            {
                "uniform_unlabeled_choice_blocks",
                "uniform_indented_unlabeled_choice_blocks",
            },
        )

    def test_removes_required_marker_after_wrapped_title(self) -> None:
        question = QuestionDraft(
            id="q_1",
            source_number=1,
            title="这是题目内容\n*",
        ...
        self.assertEqual(question.title, "这是题目内容")
