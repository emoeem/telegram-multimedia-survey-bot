...
type SessionAction =
  ...
  | { action: "set_matrix_selection"; rowId: number; columnIndex: number }
  | { action: "get_matrix_selections" }
  | { action: "clear_matrix_selections" }
  | {
      action: "complete";
    };
...
export class SurveySessionDO extends DurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (action.action === "set_matrix_selection") {
      const nextState = {
        ...state,
        matrixSelections: { ...(state.matrixSelections ?? {}), [String(action.rowId)]: action.columnIndex },
        version: state.version + 1,
        lastActivityAt: new Date().toISOString(),
      };
      await this.putState(nextState);
    ...
    if (action.action === "get_matrix_selections") {
      return Response.json({ matrixSelections: state.matrixSelections ?? {} });
    }
