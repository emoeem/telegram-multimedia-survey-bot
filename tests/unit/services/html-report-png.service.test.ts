...
describe("HTML report PNG rendering", () => {
  ...
  it("renders a 3:4 mobile viewport at 2x PNG output", async () => {
    const png = await renderHtmlReportPng(
      {} as never,
      profile,
      "Editorial + Nord",
      {},
      { layout: "editorial", theme: "nord" },
    );
    ...
    expect(page.setContent).toHaveBeenCalledWith(expect.stringContaining('data-report-layout="editorial"'), {
      waitUntil: "load",
    });
  ...
  it("rerenders an oversized page with the policy fallback DPR", async () => {
    ...
    const artifact = await renderHtmlReportArtifact(
      {} as never,
      profile,
      "Editorial + Nord",
      {},
  ...
  it("drops low-priority extension pages when the total byte policy is exceeded", async () => {
    const manyAnswers = {
      ...profile,
      metadata: {
        profile: Array.from({ length: 50 }, (_, index) => ({ label: `问题 ${index}`, value: `回答 ${index}` })),
      },
    };
    ...
    const artifact = await renderHtmlReportArtifact(
      ...
      {},
      {},
      { ...DEFAULT_REPORT_SIZE_POLICY, targetPageBytes: 4 * 1024 * 1024, maxTotalBytes: 7 * 1024 * 1024 },
    );
  ...
  it("keeps rendering later pages when one page fails twice", async () => {
    page.screenshot
      ...
      .mockRejectedValueOnce(new Error("page failed again"))
  ...
  it("records page metrics and uses JPEG fallback for a photo-heavy oversized page", async () => {
    ...
    page.screenshot
      .mockResolvedValueOnce(new Uint8Array(6 * 1024 * 1024))
      .mockResolvedValueOnce(new Uint8Array(6 * 1024 * 1024))
      .mockResolvedValueOnce(new Uint8Array(2 * 1024 * 1024));
    const artifact = await renderHtmlReportArtifact(
      {} as never,
      pictured,
      "Gallery",
  ...
  it("classifies an over-budget page as a size-limit failure with all attempts", async () => {
    const image = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAAB";
    page.screenshot
      .mockResolvedValueOnce(new Uint8Array(11 * 1024 * 1024))
      .mockResolvedValueOnce(new Uint8Array(11 * 1024 * 1024))
      .mockResolvedValueOnce(new Uint8Array(11 * 1024 * 1024))
      .mockResolvedValue(new Uint8Array([1, 2, 3]));
    const artifact = await renderHtmlReportArtifact(
