...
import {
  ...
  planResponseReportPages,
  responseReportDensity,
  validateResponseReport,
  type ResponseReport,
  type ResponseReportItem,
  type ResponseReportMedia,
...
function report(items: ResponseReportItem[]): ResponseReport {
  return {
    ...
    status: "已完成",
    respondent: "@tester",
    startedAt: "2026-08-15 09:00",
    completedAt: "2026-08-15 09:05",
    items,
  };
}
...
describe("complete survey response report", () => {
  ...
  it("renders every choice in source order with stable CSS controls", () => {
    const input = report([
      item(1, {
        ...
        rawAnswer: null,
        options: [
          { id: 11, label: "苹果", selected: false, media: [] },
          { id: 12, label: "香蕉", selected: true, media: [] },
          { id: 13, label: "橙子", selected: false, media: [] },
        ],
      }),
    ]);
    const html = buildResponseReportHtml(input);
    expect(html).toContain('data-option-id="11"');
    expect(html).toContain('data-option-id="12"');
    expect(html).toContain('data-selected="true"');
    expect(html).toContain("control radio selected");
    expect(html).not.toContain("☑");
    expect(html).not.toContain("●");
    expect(html.indexOf("苹果")).toBeLessThan(html.indexOf("香蕉"));
    expect(html.indexOf("香蕉")).toBeLessThan(html.indexOf("橙子"));
  });
    ...
    expect(
      new Set(
        pages
          .flatMap((page) => page.fragments)
          .flatMap((fragment) =>
            options.slice(fragment.optionStart ?? 0, fragment.optionEnd ?? 0).map((option) => option.id),
          ),
      ).size,
    ).toBe(24);
  ...
  it("splits wide matrices by columns without dropping any column", () => {
    ...
    const input = report([
      item(1, {
        ...
        matrixSelections: { "1": 22 },
        options: [{ id: 1, label: "矩阵行", selected: false, media: [] }],
      }),
    ]);
    const html = buildResponseReportHtml(input);
    expect([...html.matchAll(/data-column-order="(\d+)"/g)].map((match) => Number(match[1]))).toEqual(
  ...
  it("keeps question, option, and upload media associations", () => {
    const input = report([
      item(1, {
        ...
        answer: "已上传媒体文件",
        rawAnswer: null,
        questionMedia: [media(1, "question")],
        answerMedia: Array.from({ length: 7 }, (_, index) => media(index + 2, "answer")),
        options: [{ id: 1, label: "图片选项", selected: true, media: [media(20, "option")] }],
      }),
    ]);
    const html = buildResponseReportHtml(input);
    expect(html).toContain("QUESTION MEDIA");
    expect(html).toContain("OPTION MEDIA");
    expect(html).toContain("YOUR UPLOAD");
    expect((html.match(/data-media-id=/g) ?? []).length).toBe(9);
    expect(
      new Set(
        planResponseReportPages(input)
          .flatMap((page) => page.fragments)
          .flatMap((fragment) => [...(fragment.questionMedia ?? []), ...(fragment.answerMedia ?? [])])
          .map((entry) => entry.id),
      ).size,
    ).toBe(8);
  });
  ...
  it("splits media-heavy image options without duplicating or dropping media", () => {
    ...
    const input = report([
      item(1, {
        ...
        answer: "图片 A",
        rawAnswer: null,
        options: [{ id: 10, label: "图片 A", selected: true, media: optionMedia }],
      }),
    ]);
    const fragments = planResponseReportPages(input).flatMap((page) => page.fragments);
    expect(fragments.flatMap((fragment) => fragment.optionMediaById?.["10"] ?? []).map((entry) => entry.id)).toEqual(
      optionMedia.map((entry) => entry.id),
    );
  ...
  it("keeps unanswered questions visible and explicitly marked", () => {
    const html = buildResponseReportHtml(
      report([item(1, { required: false, answered: false, answerId: null, answer: "未作答", rawAnswer: null })]),
    );
  ...
  it("paginates 130 questions with images without losing coverage", () => {
    const input = report(
      Array.from({ length: 130 }, (_, index) =>
        item(index + 1, { answerMedia: index % 4 === 0 ? [media(index + 1, "answer")] : [] }),
      ),
    );
    const pages = planResponseReportPages(input);
  ...
  it("keeps all 300 questions and stored answers", () => {
    ...
    expect(
      new Set(
        planResponseReportPages(input).flatMap((page) => page.fragments.map((fragment) => fragment.item.questionId)),
      ).size,
    ).toBe(300);
  ...
  it("rejects question, option, answer, unanswered, and media corruption", () => {
    expect(() => validateResponseReport(report([item(1), item(2, { questionId: 1 })]))).toThrow(
      /duplicate question ID/,
    );
    expect(() => validateResponseReport(report([item(1, { answer: "被修改", rawAnswer: "原始答案" })]))).toThrow(
      /Answer integrity/,
    );
    expect(() =>
      validateResponseReport(
        report([item(1, { answered: false, answerId: null, answer: "伪造答案", rawAnswer: null })]),
      ),
    ).toThrow(/Unanswered state/);
    expect(() =>
      validateResponseReport(
        report([
          item(1, {
            ...
            rawAnswer: null,
            options: [
              { id: 1, label: "A", selected: true, media: [] },
              { id: 1, label: "B", selected: false, media: [] },
            ],
          }),
        ]),
      ),
    ).toThrow(/Option coverage/);
    expect(() =>
      validateResponseReport(
        report([item(1, { questionMedia: [media(1, "question")], answerMedia: [media(1, "answer")] })]),
      ),
    ).toThrow(/Media association/);
    expect(() => validateResponseReport(report([item(1, { questionMedia: [media(2, "answer")] })]))).toThrow(
      /Media association/,
    );
  });
  ...
  it("escapes untrusted content", () => {
    const html = buildResponseReportHtml(
      report([item(1, { answer: "<img onerror=alert(1)>", rawAnswer: "<img onerror=alert(1)>" })]),
    );
