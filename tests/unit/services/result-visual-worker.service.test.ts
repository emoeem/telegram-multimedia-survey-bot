  ...
  const db = {
    prepare: vi.fn((statementSql: string) => {
      ...
      const statement = {
        ...
        first: vi.fn(async () => {
          if (statementSql.includes("FROM render_jobs"))
            return {
              id: 8,
              result_profile_id: 4,
              template_id: 6,
          ...
          if (statementSql.includes("FROM survey_responses"))
            return {
              id: 3,
              survey_id: 2,
              user_id: 7,
          ...
          if (statementSql.includes("FROM visual_templates"))
...
describe("result visual queue worker", () => {
  ...
  it("delivers mobile previews, lossless pages, and an A4 archive", async () => {
    ...
    renderHtmlReportArtifact.mockResolvedValue({
      ...
      archivePdf,
      archivePdfSize: archivePdf.byteLength,
      totalPages: 2,
      totalBytes: 2,
      deliveryMode: "album",
      failures: [],
    ...
    expect(sendDocument).toHaveBeenNthCalledWith(
      3,
      "token",
      99,
      "result-report-8-archive.pdf",
      archivePdf,
      "application/pdf",
    );
    expect(sql.some((statement) => statement.includes("status = 'completed'"))).toBe(true);
  ...
  it("continues after an individual report page delivery failure", async () => {
    ...
    sendPhoto
      .mockRejectedValueOnce(new Error("page one failed"))
