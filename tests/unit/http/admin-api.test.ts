...
describe("handleAdminApi authentication and permissions", () => {
  ...
  it("rejects methods outside GET/POST/PUT/PATCH/DELETE with 405", async () => {
    repositoryMocks.getUserByTelegramId.mockResolvedValue(ADMIN);
    const { db } = makeDb();
    const response = await handleAdminApi(
      apiRequest("/api/admin/surveys", { method: "OPTIONS", userId: "111" }),
      makeEnv(db),
    );
      ...
      items: [
        {
          id: 31,
          statusLabel: "已完成",
          respondent: { telegramUserId: 999, username: "hidden", firstName: "Hidden", lastName: null },
          participantKey: null,
        },
      ],
    });
...
describe("handleAdminApi write endpoints", () => {
  ...
  it("lists users for admins only with tags and response counts", async () => {
    repositoryMocks.getUserByTelegramId.mockResolvedValue(OWNER);
    const forbidden = await handleAdminApi(apiRequest("/api/admin/users", { userId: "222" }), makeEnv(makeDb().db));
    expect(forbidden.status).toBe(403);
    ...
    const ok = await handleAdminApi(apiRequest("/api/admin/users", { userId: "111" }), makeEnv(makeDb().db));
    expect(ok.status).toBe(200);
    const body = (await ok.json()) as {
      items: Array<{ telegramUserId: number; tags: string[]; completedResponses: number }>;
    };
  ...
  it("adds and removes user tags for admins", async () => {
    ...
    expect(repositoryMocks.addUserTag).toHaveBeenCalledWith(harness.db, { userId: 5, tag: "vip", createdBy: ADMIN.id });
  ...
  it("bans and unbans users for admins with audit entries", async () => {
    repositoryMocks.getUserByTelegramId.mockResolvedValue(ADMIN);
    repositoryMocks.getUserById.mockResolvedValue({
      id: 5,
      telegramUserId: 8699777292,
    ...
    const banHarness = writableDraftDb();
    const banned = await handleAdminApi(
      apiRequest("/api/admin/users/5/ban", {
        method: "POST",
        userId: "111",
        body: { banned: true, reason: " 恶意刷屏 " },
      }),
      makeEnv(banHarness.db),
    );
    expect(banned.status).toBe(200);
    expect(await banned.json()).toEqual({ ok: true, banned: true });
    expect(repositoryMocks.setUserBan).toHaveBeenCalledWith(banHarness.db, 5, {
      banned: true,
      bannedBy: ADMIN.id,
      reason: "恶意刷屏",
    });
    expect(repositoryMocks.cancelActiveResponsesForUser).toHaveBeenCalledWith(banHarness.db, 5);
    expect(banHarness.sqlLog.some((sql) => sql.includes("INSERT INTO audit_logs"))).toBe(true);
    ...
    repositoryMocks.getUserById.mockResolvedValue({
      ...
      systemRole: "participant",
      botStartedAt: null,
      bannedAt: "2026-08-30T10:00:00.000Z",
      bannedBy: 1,
      banReason: "恶意刷屏",
      createdAt: "",
      updatedAt: "",
    });
    const unbanHarness = writableDraftDb();
    const unbanned = await handleAdminApi(
      apiRequest("/api/admin/users/5/ban", {
        method: "POST",
        userId: "111",
        body: { banned: false },
      }),
      makeEnv(unbanHarness.db),
    );
    expect(unbanned.status).toBe(200);
    expect(await unbanned.json()).toEqual({ ok: true, banned: false });
    expect(repositoryMocks.setUserBan).toHaveBeenLastCalledWith(unbanHarness.db, 5, {
      banned: false,
      bannedBy: ADMIN.id,
      reason: null,
    });
    expect(repositoryMocks.cancelActiveResponsesForUser).toHaveBeenCalledTimes(1);
    expect(unbanHarness.sqlLog.some((sql) => sql.includes("INSERT INTO audit_logs"))).toBe(true);
  });

  it("blocks non-admins from banning users and protects admin accounts", async () => {
    repositoryMocks.getUserByTelegramId.mockResolvedValue(OWNER);
    const forbidden = await handleAdminApi(
      apiRequest("/api/admin/users/5/ban", { method: "POST", userId: "222", body: { banned: true } }),
      makeEnv(makeDb().db),
    );
    expect(forbidden.status).toBe(403);
    expect(repositoryMocks.setUserBan).not.toHaveBeenCalled();
    ...
    const protectedResult = await handleAdminApi(
      apiRequest("/api/admin/users/1/ban", { method: "POST", userId: "111", body: { banned: true } }),
      makeEnv(makeDb().db),
    );
    expect(protectedResult.status).toBe(400);
    expect(await protectedResult.json()).toMatchObject({ code: "cannot_ban_admin" });
    expect(repositoryMocks.setUserBan).not.toHaveBeenCalled();
  });
  ...
  it("dashboard includes report delivery and audit summaries", async () => {
    ...
    const response = await handleAdminApi(apiRequest("/api/admin/dashboard", { userId: "111" }), makeEnv(harness.db));
    expect(response.status).toBe(200);
    ...
    restore.firstOn("FROM survey_versions", {
      snapshot_json: JSON.stringify({
        schema: {
          ...
          survey: {
            title: "历史版本问卷",
            questions: [{ id: "q1", type: "text", title: "旧题", required: true, order: 1, options: [], media: [] }],
  ...
  it("filters responses by date range", async () => {
    ...
    expect(response.status).toBe(200);
    expect(
      harness.sqlLog.some((sql) => sql.includes("date(r.started_at) >= ?") && sql.includes("date(r.started_at) <= ?")),
    ).toBe(true);
  });
  ...
  it("returns 404 for unknown write subpaths and 400 for malformed bodies", async () => {
    ...
    const request = new Request("https://example.test/api/admin/surveys/5", {
      method: "PATCH",
      headers: { "x-telegram-user-id": "111", "x-dev-auth-secret": DEV_AUTH_SECRET },
      body: "not-json",
    });
