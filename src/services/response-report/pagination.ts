...
function optionChunkSize(item: ResponseReportItem): number {
  if (item.type === "matrix")
    return Math.max(4, Math.min(9, Math.floor(45 / Math.max(5, item.matrixColumns?.length ?? 5)) + 2));
...
function splitItem(item: ResponseReportItem): ReportFragment[] {
  ...
  if (item.options.length) {
    if (item.type === "matrix") {
      ...
      for (let columnStart = 0; columnStart < columnCount; columnStart += 10) {
        for (let rowStart = 0; rowStart < item.options.length; rowStart += rowChunkSize)
          fragments.push({
            item,
            continuation: fragments.length > 0,
            continuesAfter: true,
            title: fragments.length ? "" : item.title,
            description: "",
            questionMedia: [],
            answerMedia: [],
            optionStart: rowStart,
            optionEnd: Math.min(item.options.length, rowStart + rowChunkSize),
            matrixColumnStart: columnStart,
      ...
      for (let optionIndex = 0; optionIndex < item.options.length; optionIndex += 1) {
        ...
        for (const chunk of chunks.length ? chunks : [[]])
          fragments.push({
            ...
            optionMediaById: { [String(option.id)]: chunk },
          });
      }
    } else {
      const chunkSize = optionChunkSize(item);
      for (let start = 0; start < item.options.length; start += chunkSize)
        fragments.push({
          item,
          continuation: fragments.length > 0,
          continuesAfter: true,
          ...
          answerMedia: [],
          optionStart: start,
          optionEnd: Math.min(item.options.length, start + chunkSize),
        });
    }
  } else {
    for (const answer of splitTextLossless(item.answer, 920))
      fragments.push({
        item,
        continuation: fragments.length > 0,
        continuesAfter: true,
        ...
        description: "",
        answer,
        answerStart: fragments.length === 0,
        questionMedia: [],
        answerMedia: [],
      });
  }
  ...
  if (questionMediaChunks.length) {
    const mediaFragments = questionMediaChunks.map(
      (chunk, index) =>
        ({
          item,
          continuation: index > 0,
          continuesAfter: true,
          title: index ? "" : headingTitle,
          description: index ? "" : headingDescription,
          questionMedia: chunk,
          answerMedia: [],
          optionStart: 0,
          optionEnd: 0,
        }) satisfies ReportFragment,
    );
  ...
  for (const chunk of mediaChunks(item.answerMedia, 3))
    fragments.push({
      item,
      continuation: fragments.length > 0,
      continuesAfter: true,
      ...
      questionMedia: [],
      answerMedia: chunk,
      optionStart: item.options.length,
      optionEnd: item.options.length,
    });
  if (!fragments.length)
    fragments.push({ item, continuation: false, continuesAfter: false, questionMedia: [], answerMedia: [] });
...
export function estimateResponseFragmentHeight(fragment: ReportFragment, density: ResponseReportDensity): number {
  ...
  let height =
    70 +
    lines(fragment.title ?? item.title, 62) * 25 +
    (fragment.description ? lines(fragment.description, 74) * 21 : 0);
  height += (fragment.questionMedia?.length ?? 0) * 190;
  if (fragment.answer !== undefined) height += 42 + lines(fragment.answer, 76) * 23;
  if (item.type === "matrix" && options.length)
    height += 56 + options.reduce((sum, option) => sum + Math.max(42, lines(option.label, 22) * 20 + 18), 0);
  else
    height += options.reduce(
      (sum, option) =>
        sum +
        Math.max(38, lines(option.label, 46) * 20 + 14) +
        (fragment.optionMediaById?.[String(option.id)] ?? option.media).length * 160,
      0,
    );
...
export function planResponseReportPages(report: ResponseReport): ResponseReportPage[] {
  ...
  for (const item of report.items)
    for (const fragment of splitItem(item)) {
      const height = estimateResponseFragmentHeight(fragment, density);
      if (height > PAGE_CONTENT_HEIGHT)
        throw new Error(`Page split validation failed: question ${item.questionId} fragment still exceeds page budget`);
      let page = pages.at(-1);
      const pageBudget = pages.length <= 1 ? PAGE_CONTENT_HEIGHT - 120 : PAGE_CONTENT_HEIGHT;
      if (!page || page.estimatedHeight + height > pageBudget) {
        page = { fragments: [], estimatedHeight: 0 };
        pages.push(page);
      }
  ...
  const planned = pages.map((page, index) => ({
    number: index + 1,
    total: pages.length,
