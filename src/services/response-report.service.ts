...
export type {
  ...
  ResponseReportPage,
  ResponseReportRenderResult,
} from "./response-report/model";
export { planResponseReportPages, responseReportDensity } from "./response-report/pagination";
export {
  validateResponseReportPages,
  validateResponseReportSource as validateResponseReport,
} from "./response-report/validation";

const TARGET_PAGE_BYTES = 5 * 1024 * 1024;
const HARD_MAX_PAGE_BYTES = 10 * 1024 * 1024;
const TARGET_TOTAL_BYTES = 40 * 1024 * 1024;

async function waitForResponseReportAssets(
...
async function assertNoVisualOverflow(
  page: Awaited<ReturnType<Awaited<ReturnType<typeof puppeteer.launch>>["newPage"]>>,
): Promise<void> {
  const overflowPage = await page.evaluate(
    "(()=>{for(const page of document.querySelectorAll('.page')){const content=page.querySelector('.page-content');if(content&&content.scrollHeight>content.clientHeight+1)return page.getAttribute('data-page')}return null})()",
  );
  if (overflowPage)
    throw new Error(`Page split validation failed: rendered page ${overflowPage} overflows its fixed canvas`);
}

async function repaginateRenderedReport(
  page: Awaited<ReturnType<Awaited<ReturnType<typeof puppeteer.launch>>["newPage"]>>,
): Promise<number> {
  ...
  if (typeof pageCount !== "number")
    throw new Error("Page split validation failed: rendered page count is unavailable");
  return pageCount;
}
