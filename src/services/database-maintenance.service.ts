...
export async function runDatabaseMaintenance(db: D1Database, now = Date.now()): Promise<DatabaseMaintenanceSummary> {
  ...
  const expiredResponses = await changes(
    db
      .prepare(
  ...
  const expiredSurveyDrafts = await changes(
    db
      .prepare(
        `DELETE FROM surveys
  ...
  const expiredTemplateDrafts = await changes(
    db
      .prepare(
  ...
  const expiredGeneratorDrafts = await changes(
    db
      .prepare(
  ...
  const expiredRenderJobs = await changes(
    db
      .prepare(
  ...
  const expiredGeneratorJobs = await changes(
    db
      .prepare(
  ...
  const expiredReportResults = await changes(
    db
      .prepare(
  ...
  const expiredExportJobs = await changes(
    db
      .prepare(
  ...
  const expiredAuditLogs = await changes(db.prepare("DELETE FROM audit_logs WHERE created_at < ?").bind(auditBefore));

  const expiredPreviewAssets = await changes(
    db
      .prepare(
  ...
  const expiredGeneratedAssets = await changes(
    db
      .prepare(
        `DELETE FROM media_assets
  ...
  const orphanAssets = await changes(
    db
      .prepare(
        `DELETE FROM media_assets
     WHERE created_at < ?
       ...
       AND NOT EXISTS (SELECT 1 FROM gallery_profile_media WHERE media_asset_id = media_assets.id)
