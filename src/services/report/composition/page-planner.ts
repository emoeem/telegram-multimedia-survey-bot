...
function splitText(value: string, maxCharacters: number): string[] {
  ...
  while (remaining.length > maxCharacters) {
    ...
    const splitAt = boundary > maxCharacters * 0.55 ? boundary + 1 : maxCharacters;
...
function responseHeight(
  featured: ReportAnswerItem | undefined,
  editorial: ReportAnswerItem[],
  compact: ReportAnswerItem[],
): number {
  return (
    95 +
    (featured ? textHeight(featured.value, 100) : 0) +
    (editorial.length ? Math.max(...editorial.map((item) => textHeight(item.value, 90))) : 0) +
    (compact.length ? 170 * Math.ceil(compact.length / 4) : 0)
  );
}
...
export function planReportPages(
  ...
  if (view.scores.length || view.tags.length)
    candidates.push({
      id: "overview",
      kind: "overview",
      priority: 90,
      estimatedHeight: view.scores.length >= 3 ? 430 : 260,
    });
  if (content.featuredInsight)
    candidates.push({
      id: "featured",
      kind: "featured",
      priority: 90,
      estimatedHeight: Math.min(330, textHeight(content.featuredInsight.text, 170)),
      item: content.featuredInsight,
    });
  for (const item of content.analysis.flatMap((entry) => splitEditorialItem(entry))) {
    candidates.push({
      id: `analysis-${item.id}`,
      kind: "analysis",
      priority: /Core Personality/i.test(item.title) ? 95 : /Contradiction/i.test(item.title) ? 90 : 85,
      estimatedHeight: Math.min(budget, textHeight(item.text, 170)),
      items: [item],
    });
  }
  if (content.quotes.length)
    candidates.push({
      ...
      kind: "quotes",
      priority: 80,
      estimatedHeight: Math.min(520, 120 + Math.max(...content.quotes.map((item) => textHeight(item.text, 80)))),
      items: content.quotes,
    });
  if (content.featuredAnswer) {
    candidates.push({
      id: "featured-response",
      kind: "responses",
      priority: 75,
      estimatedHeight: responseHeight(content.featuredAnswer, [], []),
      featured: content.featuredAnswer,
      editorial: [],
      compact: [],
    });
  }
  if (content.editorialAnswers.length) {
    candidates.push({
      id: "editorial-responses",
      kind: "responses",
      priority: 70,
      estimatedHeight: responseHeight(undefined, content.editorialAnswers, []),
      editorial: content.editorialAnswers,
      compact: [],
    });
  }
  ...
  for (let offset = 0; offset < compactAnswers.length; offset += 8) {
    ...
    candidates.push({
      id: `compact-responses-${offset / 8 + 1}`,
      kind: "responses",
      priority: 55,
      estimatedHeight: responseHeight(undefined, [], compact),
      editorial: [],
      compact,
    });
  }
  ...
  for (let offset = 0; offset < galleryItems.length; offset += policy.maxImagesPerPage) {
    ...
    candidates.push({
      id: `gallery-${offset / policy.maxImagesPerPage + 1}`,
      kind: "gallery",
      priority: offset === 0 ? 75 : 50,
      estimatedHeight: 110 + rows * 300,
      items,
    });
  }
  ...
  const startPage = (block: ReportPageBlock): ReportPage => ({
  ...
  for (const block of candidates) {
    ...
    if (
      current.blocks.some((entry) => pageKind(entry) !== current!.kind) &&
      !(current.kind === "cover" && current.blocks.every((entry) => entry.kind === "hero" || entry.kind === "overview"))
    )
      current.kind = "mixed";
  }
  if (current?.blocks.length) pages.push(current);
  ...
  const essential = pages.filter((page) =>
    page.blocks.some((block) => block.kind === "hero" || block.kind === "verdict"),
  );
  const essentialIds = new Set(essential.map((page) => page.id));
  const selected = pages
    .filter((page) => !essentialIds.has(page.id))
    .sort((left, right) => right.priority - left.priority)
    .slice(0, Math.max(0, policy.maxPages - essential.length));
...
export function selectGalleryForPolicy(
  items: ReportGalleryItem[],
  mode: PreparedReportContent["densityMode"],
): ReportGalleryItem[] {
