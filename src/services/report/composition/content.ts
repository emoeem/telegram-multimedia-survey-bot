...
function excerpt(value: string, length: number): string {
  ...
  const boundary = Math.max(
    clean.lastIndexOf("。", length),
    clean.lastIndexOf("！", length),
    clean.lastIndexOf("？", length),
  );
  return `${clean.slice(0, boundary > length * 0.55 ? boundary + 1 : length).trim()}…`;
}
...
function isConsumed(answer: ReportAnswerItem, consumedSources: Set<string>, consumedText: string[]): boolean {
  ...
  return consumedText.some(
    (value) =>
      candidate === value || (candidate.length > 60 && (value.includes(candidate) || candidate.includes(value))),
  );
}

function verdictTitle(view: ReportViewModel): string {
  const title =
    view.hero.subtitle && !/问卷完成|自动整理|根据本次问卷/.test(view.hero.subtitle)
      ? view.hero.subtitle
      : view.hero.title;
...
export function prepareReportContent(view: ReportViewModel): PreparedReportContent {
  ...
  const compactLimit =
    densityMode === "compact" ? 8 : densityMode === "standard" ? 10 : densityMode === "extended" ? 8 : 6;
  const editorialAnswers = afterFeatured
    .filter((item) => item.value.length >= 55 && item.value.length <= 2400)
    .slice(0, editorialLimit);
  const editorialIds = new Set(editorialAnswers.map((item) => item.id));
  const compactAnswers = remainingAnswers
    .filter((item) => item.id !== featuredAnswer?.id && !editorialIds.has(item.id))
  ...
  const summaryCandidate =
    view.summary && !analysis.some((item) => normalizeReportText(view.summary).includes(normalizeReportText(item.text)))
      ? view.summary
  ...
  return {
    ...
    verdict: {
      ...
      pillars: ["CORE DRIVE", "RELATIONSHIP NEED", "INNER TENSION"].map((label, index) => ({
        label,
        value: pillarSources[index] ?? fallbackPillars[index]!,
      })),
    },
  };
}
