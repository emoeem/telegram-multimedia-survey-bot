...
export interface DataBlockContext {
  accent: string;
...
export function renderMetricGridBlock(scores: ReportScore[], context: DataBlockContext): string {
  return scores
    .slice(0, 4)
    .map((score) => renderProgressBlock(score, context))
    .join("");
}
...
export function renderProgressBars(scores: ReportScore[], context: DataBlockContext): string {
  return scores
    .map(
      (score) =>
        `<div class="bar-row"><span>${context.escape(score.label)}</span><div class="bar-track"><i style="width:${score.percentage}%"></i></div><b>${context.escape(String(score.value))}</b></div>`,
    )
    .join("");
}
