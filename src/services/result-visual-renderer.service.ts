...
function renderText(element: VisualTemplateElement, context: RenderContext, report?: VisualReportLayout): string {
  ...
  const lines = wrapText(
    interpolate(element.value ?? "", context),
    width || 10_000,
    fontSize,
    element.maxLines,
    element.overflow,
...
function renderElement(
  element: VisualTemplateElement,
  context: RenderContext,
  images: Record<string, string>,
  report?: VisualReportLayout,
): string {
  if (!isVisible(element, context)) return "";
  if (element.type === "text" || element.type === "badge" || element.type === "tag" || element.type === "icon")
    return renderText(element, context, report);
...
function renderReportSection(
  ...
  if (section.visibleIf && !isVisible(section as unknown as VisualTemplateElement, context))
  ...
  if (section.type === "gallery") {
    ...
    items.forEach((item, index) => {
      ...
      const image =
        images[`${path}.${index}`] ?? (typeof item === "string" && item.startsWith("data:image/") ? item : null);
      if (!image) return;
      ...
      const max =
        Number(
          section.max === undefined
            ? objectValue(item, "max", 10)
            : typeof section.max === "number"
              ? section.max
              : resolvePath(context, expressionPath(section.max) ?? ""),
        ) || 10;
      const barX = left;
...
function renderReportSections(
  ...
  for (const section of template.sections ?? []) {
    const rendered = renderReportSection(
      section,
      context,
      images,
      paddingX,
...
function renderBackground(
  ...
  if (background.type === "solid")
    return {
      defs: "",
      markup: `<rect width="${width}" height="${height}" fill="${escapeAttribute(interpolate(background.color, context))}"/>${overlay}${card}`,
    };
  ...
  const source =
    background.type === "telegram_asset"
...
export function renderResultVisualSvg(
  ...
  const height =
    template.height === "auto"
      ? Math.min(MAX_RESULT_VISUAL_HEIGHT, Math.max(1, Math.ceil(report.height)))
      : template.height;
