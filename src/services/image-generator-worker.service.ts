...
export interface ImageGeneratorJobMessage {
  kind: "image_generator";
  jobId: number;
}
export function isImageGeneratorJobMessage(value: unknown): value is ImageGeneratorJobMessage {
  return Boolean(
    value &&
    typeof value === "object" &&
    (value as Record<string, unknown>).kind === "image_generator" &&
    Number.isSafeInteger((value as Record<string, unknown>).jobId),
  );
}
...
function generatorProfile(
  ...
  for (const question of questions) {
    ...
    if (question.type === "multiple" && Array.isArray(value))
      tags.push(...value.filter((item): item is string => typeof item === "string"));
  ...
  return {
    ...
    tags,
    images,
    metadata: { profile, status, summary: summaries.join("\n\n"), gallery },
    schemaVersion: 1,
  };
}
...
function adaptTemplate(
  ...
  contrastMode: ReportContrastMode,
) {
  ...
  const replace = (value?: string): string | undefined =>
    value?.replace(/\{\{\s*([A-Za-z0-9_-]+)\s*\}\}/g, (_m, key: string) => {
  ...
  if (template.sections) {
    template.sections = template.sections.map(
      (section) =>
        ({
          ...section,
  ...
  return parseVisualTemplateDefinition(
    JSON.stringify(applyReportPresentation(template, backgroundAssetId, contrastMode)),
  );
}
export async function enqueueImageGeneratorJob(
  db: D1Database,
  queue: Queue,
  input: {
    generatorId: number;
    ...
    values: Record<string, GeneratorInputValue>;
    backgroundAssetId: number | null;
    chatId: number;
    userId: number;
  },
): Promise<number> {
  const [questions, now] = await Promise.all([
    listGeneratorQuestions(db, input.generatorId),
    Promise.resolve(new Date().toISOString()),
  ]);
  const imageVariables = new Set(
    questions.filter((question) => question.type === "image").map((question) => question.variableName),
  );
  ...
  const reportResultId = await createReportResult(db, {
  ...
  const r = await db
    .prepare(
      "INSERT INTO image_generator_jobs(generator_id,template_id,template_version,input_json,background_asset_id,chat_id,user_id,report_result_id,created_at) VALUES(?,?,?,?,?,?,?,?,?)",
    )
    .bind(
      input.generatorId,
      input.templateId,
  ...
  await queue.send({ kind: "image_generator", jobId: id } satisfies ImageGeneratorJobMessage);
  return id;
}
export async function processImageGeneratorMessage(
  env: { DB: D1Database; BOT_TOKEN: string },
  body: unknown,
): Promise<void> {
  if (!isImageGeneratorJobMessage(body)) return;
  const job = await env.DB.prepare("SELECT * FROM image_generator_jobs WHERE id=?")
    .bind(body.jobId)
    .first<Record<string, unknown>>();
  if (!job) return;
  const claim = await env.DB.prepare(
    "UPDATE image_generator_jobs SET status='processing',attempts=attempts+1,error_message=NULL WHERE id=? AND status='queued'",
  )
    .bind(body.jobId)
    .run();
  if (!claim.meta?.changes) return;
  const [version, generator, questions] = await Promise.all([
    getVisualTemplateVersion(env.DB, Number(job.template_id), Number(job.template_version)),
    getGenerator(env.DB, Number(job.generator_id)),
    listGeneratorQuestions(env.DB, Number(job.generator_id)),
  ]);
  if (!version) throw new Error("generator template version not found");
  if (!generator) throw new Error("report generator not found");
  const values = JSON.parse(String(job.input_json)) as Record<string, GeneratorInputValue>;
  const profile = generatorProfile(values, questions, generator.name);
  const requestedBackground = typeof job.background_asset_id === "number" ? job.background_asset_id : null;
  const backgroundAssetId = requestedBackground ?? generator.reportBackgroundAssetId;
  const template = adaptTemplate(version.definitionJson, questions, backgroundAssetId, generator.reportContrastMode);
  const images = await resolveResultVisualImages(env.DB, env.BOT_TOKEN, template, profile);
  const [{ renderResultVisualPng }, { RESULT_VISUAL_FONTS }, { RESULT_VISUAL_WASM }] = await Promise.all([
    import("./result-visual-renderer.service"),
    import("./result-visual-font"),
    import("./result-visual-wasm"),
  ]);
  const png = await renderResultVisualPng(template, profile, {
    wasmModule: RESULT_VISUAL_WASM,
    fontBuffers: RESULT_VISUAL_FONTS,
    images,
  });
  await sendPhoto(env.BOT_TOKEN, Number(job.chat_id), png, `🎉 ${generator.name} 已生成`);
  await env.DB.prepare("UPDATE image_generator_jobs SET status='completed',completed_at=? WHERE id=?")
    .bind(new Date().toISOString(), body.jobId)
    .run();
}
export async function retryImageGeneratorJob(
  db: D1Database,
  id: number,
  error: string,
  terminal: boolean,
  ...
  await db
    .prepare("UPDATE image_generator_jobs SET status=?,error_message=?,completed_at=? WHERE id=?")
    .bind(terminal ? "failed" : "queued", error.slice(0, 500), terminal ? new Date().toISOString() : null, id)
    .run();
}
