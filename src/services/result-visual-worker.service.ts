...
import { claimRenderJob, completeRenderJob, getRenderJobById } from "../db/repositories/result-visual.repository";
...
async function deliverReportArtifact(
  botToken: string,
  chatId: number,
  jobId: number,
  artifact: ReportArtifact,
  regenerateCallback: string,
): Promise<number> {
    ...
    try {
      await sendPhotoAlbum(
        ...
        artifact.pages.map((page, index) => ({
          bytes: page.bytes,
          ...(index === 0 ? { caption: `🎉 你的分析报告 · 共 ${artifact.totalPages} 页` } : {}),
        })),
      );
  ...
  for (const [index, page] of albumDelivered ? [] : artifact.pages.entries()) {
    ...
    const replyMarkup =
      index === artifact.pages.length - 1
        ? { inline_keyboard: [[{ text: "🔄 重新生成", callback_data: regenerateCallback }]] }
        : undefined;
    try {
      if (page.deliveryMode === "document" || page.size > 10 * 1024 * 1024)
        throw new Error("page selected document delivery by size policy");
      ...
      try {
        await sendDocument(
          botToken,
          chatId,
          `result-report-${jobId}-${String(index + 1).padStart(2, "0")}.${page.format === "jpeg" ? "jpg" : "png"}`,
          page.bytes,
          page.type,
        );
        delivered += 1;
        ...
        pageFailures.push(
          `${page.id}: ${documentError instanceof Error ? documentError.message : String(documentError)}`,
        );
        console.error("[ReportPage] delivery failed", {
          jobId,
          pageId: page.id,
          mode: page.deliveryMode,
          sendPhoto: photoError,
          sendDocument: documentError,
        });
      }
    }
  }
  if (artifact.archivePdf) {
    for (const [index, page] of artifact.pages.entries()) {
      try {
        await sendDocument(
          botToken,
          chatId,
          `result-report-${jobId}-hd-${String(index + 1).padStart(2, "0")}.${page.format === "jpeg" ? "jpg" : "png"}`,
          page.bytes,
          page.type,
        );
      } catch (error) {
    ...
    try {
      await sendDocument(
        botToken,
        chatId,
        `result-report-${jobId}-archive.pdf`,
        artifact.archivePdf,
        "application/pdf",
      );
    } catch (error) {
  ...
  if (artifact.failures.length || pageFailures.length) {
    const failedPageIds = [
      ...artifact.failures.map((failure) => failure.pageId),
      ...pageFailures.map((failure) => failure.split(":", 1)[0]!),
    ];
    try {
      await sendMessage(
        botToken,
        chatId,
        `⚠️ 报告有 ${failedPageIds.length} 个页面生成或发送失败（${failedPageIds.join("、")}）。已送达页面可以正常查看，请稍后重新生成缺失内容。`,
      );
    } catch (error) {
...
export async function processResultVisualMessage(env: ResultVisualWorkerEnvironment, body: unknown): Promise<void> {
  ...
  if (templateRecord?.type === "report" && env.BROWSER) {
    try {
      ...
      throw new Error(
        `Multi-page HTML report rendering failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  } else {
  ...
  if (job.chatId !== null) {
    ...
    try {
      await sendPhoto(env.BOT_TOKEN, job.chatId, png, "🎉 你的结果卡已生成", {
        inline_keyboard: [[{ text: "🔄 重新生成", callback_data: `rv:regenerate:${profileRecord.responseId}` }]],
      });
    } catch (photoError) {
