...
import {
  renderEditorialAnalysisBlock,
  renderFeaturedInsightBlock,
  renderHeroBlock,
  renderQuoteBlock,
} from "./report/blocks/content";
...
export function selectReportLayout(view: ReportViewModel, options: ReportLayoutOptions = {}): ReportLayout {
  ...
  for (const value of [
    view.summary,
    ...view.insights.map((item) => item.text),
    ...view.quotes.map((item) => item.text),
  ]) {
  ...
  return (
    (Object.entries(weights) as Array<[ReportLayout, number]>).sort((left, right) => right[1] - left[1])[0]?.[0] ??
    "editorial"
  );
}
...
export function buildReportViewModel(
  ...
  const profileItems = rawProfile
    .map((item, index) => {
      ...
      const matchingField = Object.entries(profile.fields).find(
        ([, field]) => text(field.value).trim() === value.trim(),
      );
    ...
    .slice(0, 250);
  const scores = profile.stats
    .filter((stat) => Number.isFinite(stat.value) && Number.isFinite(stat.max) && Number(stat.max) > 0)
    .slice(0, 12)
    .map((stat) => {
      const max = stat.max!;
      ...
      return {
        key: stat.id,
        label: limitText(stat.label, 80),
        value: stat.value,
        max,
        percentage,
        level: scoreLevel(percentage),
        description: scoreDescription(percentage),
      };
    });
  const longTexts = Object.entries(profile.fields)
    ...
    .map(([id, field], index) => {
      ...
      return {
        id: `text-${index}`,
        sourceId: id,
        title: matchingProfile?.label || `回答 ${id.replace(/^question_/, "#")}`,
        text: value,
      };
    });
  ...
  const gallery: ReportGalleryItem[] = Object.entries(images)
    .filter(
      ([key, value]) =>
        !key.startsWith("template.background") && (value.startsWith("data:image/") || value.startsWith("/api/")),
    )
    .slice(0, 60)
    .map(([, url], index) => {
      ...
      return {
        ...inspectReportImage(url),
        url,
        byteSize: reportImageByteSize(url),
        sourceHash: reportImageHash(url),
        ...(text(sourceRecord.caption) ? { caption: limitText(text(sourceRecord.caption), 180) } : {}),
        ...(text(sourceRecord.questionTitle)
          ? { questionTitle: limitText(text(sourceRecord.questionTitle), 180) }
          : {}),
      };
    });
  const avatar =
    ["result.images.avatar", "result.images.portrait", "result.images.profilePhoto", "result.images.front_image"]
      .map((key) => images[key])
      .find((value) => value?.startsWith("data:image/") || value?.startsWith("/api/")) ?? gallery[0]?.url;
  const requestedLayout = asReportLayout(metadata.layout ?? metadata.reportLayout);
  const requestedTheme =
    typeof metadata.theme === "string" && metadata.theme in reportThemes ? (metadata.theme as ReportTheme) : undefined;
  return {
    hero: {
      title: limitText(profile.title ?? "问卷结果报告", 160),
      subtitle: limitText(profile.subtitle ?? "根据本次问卷回答生成", 300),
      tags: profile.tags.slice(0, 20),
      ...(avatar ? { avatar, coverImage: avatar } : {}),
    },
    scores,
    charts: {
      radar: scores.slice(0, 6).map((score) => ({ label: score.label, value: score.percentage })),
      bars: scores.slice(0, 8),
    },
    tags: profile.tags.slice(0, 20),
    insights,
    quotes,
    gallery,
    summary: limitText(text(metadata.summary), 8000),
    profile: profileItems,
    contentStats: {
      answerCount: rawProfile.length,
      imageCount: Object.keys(images).filter((key) => !key.startsWith("template.background")).length,
...
/* Layouts that override the theme tokens must paint their own background,
 * otherwise a dark theme shows through around the light layout surface. */
[data-report-layout]{background:var(--report-bg);min-height:100vh}
...
function renderComposition(
    ...
    hero: renderHeroBlock(
      {
        ...
        thesis,
        tags: view.hero.tags,
        ...(view.hero.avatar ? { avatar: view.hero.avatar } : {}),
        ...(primaryScore ? { primaryScore } : {}),
      },
      { escape: escapeHtml, limit: limitText },
    ),
    overview: renderOverview(view, colors),
    featured: renderFeaturedInsightBlock(content.featuredInsight, { escape: escapeHtml, limit: limitText }),
    analysis: renderEditorialAnalysisBlock(content.analysis, { escape: escapeHtml, limit: limitText }),
    quotes: renderQuoteBlock(content.quotes, { escape: escapeHtml, limit: limitText }),
    responses: renderSelectedResponses(content, { escape: escapeHtml }),
    gallery: renderGalleryBlock(view.gallery, { heroUrl: view.hero.avatar, escape: escapeHtml }),
  ...
  const html = composition.regions
    .map(
      (region) =>
        `<div class="composition-region composition-region-${region.role}" data-region="${region.id}">${region.blocks
          .map((spec) => {
            const markup = blocks[spec.kind] ?? "";
            return markup
              ? `<div class="composition-block presentation-${spec.presentation} reading-${spec.readingWidth} emphasis-${spec.emphasis}" data-block="${spec.kind}">${markup}</div>`
              : "";
          })
          .join("")}</div>`,
    )
...
export function buildHtmlReport(
  ...
  const layout =
    options.layout ?? view.meta.layout ?? layoutHintFromTemplateName(templateName) ?? selectReportLayout(view, options);
  ...
  const meta = [view.meta.surveyTitle, view.meta.submittedAt, view.meta.reportId]
    ...
    .join(" · ");
  return `<!doctype html><html><head><meta charset="utf-8"><style>${themeCss(theme)}${reportCss()}</style></head><body><div class="report-layout-${layout} density-${composition.density}" data-report-layout="${layout}" data-report-theme="${theme.id}" data-report-mode="${theme.dark ? "dark" : "light"}"><main class="page">${composition.html}${renderMetadataBlock([meta], { escape: escapeHtml }) || `<footer class="report-metadata">本报告由问卷回答自动整理生成</footer>`}</main></div></body></html>`;
}
...
export function buildResponsiveCompositionReport(
  ...
  const metaLine = [meta.surveyTitle, meta.completedAt, meta.reportId]
    .filter(Boolean)
...
function partialResponses(
  ...
  block: Extract<ReportPageBlock, { kind: "responses" }>,
): PreparedReportContent {
...
function renderAnswerValue(item: ReportAnswerItem, context: { escape(value: string): string }): string {
...
function renderFullTranscript(view: ReportViewModel, context: { escape(value: string): string }): string {
...
function renderPlannedBlock(
  ...
  switch (block.kind) {
    ...
    case "overview":
      return renderOverview(view, colors);
    case "featured":
      return renderFeaturedInsightBlock(block.item, context);
    case "analysis":
      return renderEditorialAnalysisBlock(block.items, context);
    case "quotes":
      return renderQuoteBlock(block.items, context);
    case "responses":
      return renderSelectedResponses(partialResponses(content, block), { escape: escapeHtml });
    case "gallery":
      return renderGalleryBlock(block.items, { escape: escapeHtml });
    case "verdict":
      return renderFinalVerdictBlock(content.verdict, primaryScore, context);
  }
}
...
function htmlShell(
  ...
  const folio = page
    ? `<div class="page-folio">${escapeHtml(page.id.toUpperCase())} / ${String(totalPages ?? 1).padStart(2, "0")}</div>`
    : "";
  return `<!doctype html><html${pageClass}><head><meta charset="utf-8"><style>${themeCss(theme)}${reportCss()}</style></head><body><div class="report-layout-${layout} density-${density}" data-report-layout="${layout}" data-report-theme="${theme.id}" data-report-mode="${theme.dark ? "dark" : "light"}"${page ? ` data-report-page="${page.id}" data-page-kind="${page.kind}"` : ""}><main class="page">${body}${folio}</main></div></body></html>`;
}
  ...
  const pages = planned.map((page) => ({
    ...
    html: htmlShell(
      page.blocks
        .map(
          (block) =>
            `<div class="composition-region composition-region-${block.kind}"><div class="composition-block" data-block="${block.kind}">${renderPlannedBlock(block, view, content, themeChartColors(theme))}</div></div>`,
        )
        .join(""),
      layout,
      theme,
...
export async function renderHtmlReportArtifact(
  ...
  try {
    ...
    for (const planned of report.pages) {
      ...
      const imageCount =
        imageItems.length +
        (planned.page.blocks.some((block) => block.kind === "hero") && report.view.hero.avatar ? 1 : 0);
      const embeddedImageBytes =
        imageItems.reduce((sum, item) => sum + (item.byteSize ?? reportImageByteSize(item.url)), 0) +
        (planned.page.blocks.some((block) => block.kind === "hero") && report.view.hero.avatar
          ? reportImageByteSize(report.view.hero.avatar)
          : 0);
      ...
      for (let attempt = 0; attempt < maxAttempts && !rendered; attempt += 1) {
        ...
        try {
          ...
          if (embeddedImageBytes > policy.maxEmbeddedImageBytesPerPage)
            throw new Error(`embedded images exceed ${policy.maxEmbeddedImageBytesPerPage} bytes`);
          ...
          const overflow = await page.evaluate(
            "(()=>{const page=document.querySelector('.page');return page ? page.scrollHeight > page.clientHeight : true})()",
          );
          if (overflow) {
            await page.evaluate(
              "document.querySelectorAll('.composition-region').forEach((node)=>node.style.setProperty('margin-top','12px','important'));document.querySelectorAll('.editorial-section').forEach((node)=>node.style.setProperty('padding-top','14px','important'));document.querySelectorAll('.editorial-copy p,.answer').forEach((node)=>node.style.setProperty('font-size','15px','important'));",
            );
          ...
          const candidate = new Uint8Array(
            await page.screenshot(
              format === "jpeg"
                ? { type: "jpeg", quality: 88, captureBeyondViewport: false }
          ...
          if (candidate.byteLength > policy.targetPageBytes && (attempt === 0 || (imageCount > 0 && format === "png")))
            continue;
          if (candidate.byteLength > policy.hardMaxPageBytes)
            throw new Error(`page image exceeds ${policy.hardMaxPageBytes} bytes`);
          rendered = candidate;
          renderedDpr = dpr;
      ...
      if (!rendered) {
        ...
        const stage = /exceeds/.test(message)
          ? "size_limit"
        ...
        console.error("[ReportPage] render failed", {
          page: planned.page.id,
          type: planned.page.kind,
          htmlBytes: htmlByteSize,
          imageCount,
          embeddedImageBytes,
          width: policy.pageWidth,
          height: policy.maxPageHeight,
          attempts,
          error: message,
        });
        continue;
      }
      const width = Math.round(policy.pageWidth * renderedDpr);
      const height = Math.round(policy.maxPageHeight * renderedDpr);
      pages.push({
        id: planned.page.id,
        kind: planned.page.kind,
        bytes: rendered,
        ...
        type: renderedFormat === "jpeg" ? "image/jpeg" : "image/png",
        rawByteSize,
        optimizedByteSize: rendered.byteLength,
        ...(renderedFormat === "jpeg" ? { compressionLevel: 88 } : {}),
        containsImages: imageCount > 0,
        imageCount,
        embeddedImageBytes,
        htmlByteSize,
        estimatedComplexity: planned.page.estimatedHeight + imageCount * 200,
        deliveryMode: rendered.byteLength <= policy.targetPageBytes ? "photo" : "document",
        optimizationAttempts: attempts.length - 1,
        finalStatus: "ready",
      });
    }
    if (!pages.length)
      throw new Error(
        `all report pages failed: ${failures.map((failure) => `${failure.pageId}: ${failure.message}`).join("; ")}`,
      );
    ...
    if (totalBytes > policy.maxTotalBytes) {
      ...
      const essentialPageIds = new Set(
        report.pages
          .filter((entry) => entry.page.blocks.some((block) => block.kind === "hero" || block.kind === "verdict"))
          .map((entry) => entry.page.id),
      );
      const removable = selectedPages
        .filter((page) => !essentialPageIds.has(page.id))
        .sort((left, right) => (priority.get(left.id) ?? 0) - (priority.get(right.id) ?? 0));
    ...
    const deliveryMode =
      selectedPages.length === 1
        ...
        : selectedPages.length > 10
          ? "split"
          : selectedPages.every((page) => page.deliveryMode === "photo")
            ? "album"
            : "multi_document";
    return {
      pages: selectedPages,
      ...(archivePdf ? { archivePdf, archivePdfSize: archivePdf.byteLength } : {}),
...
export async function renderHtmlReportPng(
  browserBinding: BrowserWorker,
  profile: ResultProfileSnapshot,
  templateName: string,
  images: Record<string, string> = {},
