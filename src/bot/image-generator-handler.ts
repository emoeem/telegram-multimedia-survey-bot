...
export async function clearImageGeneratorInteractionState(ctx: BotContext, userId: number): Promise<void> {
  await Promise.all([ctx.cache?.delete(adminKey(userId)), ctx.cache?.delete(sessionKey(userId))]);
}
...
function isAdminState(value: unknown): value is AdminState {
  ...
  return (
    (state.kind === "new" ||
      state.kind === "new_report" ||
      state.kind === "import" ||
      state.kind === "import_style" ||
      state.kind === "question" ||
      state.kind === "background" ||
      state.kind === "report_background") &&
    Number.isSafeInteger(state.chatId) &&
    Number.isSafeInteger(state.messageId) &&
    (state.kind === "new"
      ? Number.isSafeInteger(state.templateId)
      : state.kind === "new_report" || state.kind === "import" || Number.isSafeInteger(state.generatorId))
  );
}
...
function isSession(value: unknown): value is Session {
  ...
  return (
    Number.isSafeInteger(session.generatorId) &&
...
async function showGeneratorList(ctx: BotContext, chatId: number, userId: number, messageId?: number): Promise<void> {
  ...
  await render(ctx, chatId, userId, messageId, "IMAGE_GENERATOR_LIST", "📊 报告生成\n\n选择一个已发布报告：", {
    inline_keyboard: rows,
  });
}

async function showAdminList(ctx: BotContext, chatId: number, userId: number, messageId?: number): Promise<void> {
...
async function createDemoReportGenerator(ctx: BotContext, ownerId: number): Promise<number> {
  ...
  const questions: Array<Omit<GeneratorQuestion, "id" | "generatorId" | "sortOrder">> = [
    ...
    {
      variableName: "photos",
      prompt: "上传希望展示在报告中的照片？",
      type: "image",
      required: false,
      options: [],
      settings: { minImages: 0, maxImages: 4 },
    },
    {
      variableName: "tags",
      prompt: "选择你的偏好标签？",
...
async function getOrCreateReportTemplate(
  ...
  const existing = (await listVisualTemplates(ctx.db, 100)).find(
    (template) =>
      template.name === name &&
...
async function showAdminDetail(
  ...
  const text = [
    ...
    "",
    questions.length
      ? `问题：${questions.length} 道（${Object.entries(
          questions.reduce<Record<string, number>>(
            (summary, question) => ({ ...summary, [question.type]: (summary[question.type] ?? 0) + 1 }),
            {},
          ),
        )
  ...
  await render(ctx, chatId, userId, messageId, "IMAGE_GENERATOR_DETAIL", text, {
    inline_keyboard: [
      ...
      [
        {
          text: generator.reportBackgroundAssetId ? "🖼 更换报告默认背景" : "🖼 上传报告默认背景",
          callback_data: `generator:report_background:${generatorId}`,
        },
        { text: "♻️ 清除", callback_data: `generator:report_background_clear:${generatorId}` },
      ],
...
function questionSettings(question: GeneratorQuestion): {
  minImages: number;
  maxImages: number;
  min: number;
  max: number;
...
function answerKeyboard(question: GeneratorQuestion, selected: string[] = []): InlineKeyboardMarkup | null {
    ...
    return {
      inline_keyboard: [
        ...question.options.map((option, index) => [
          { text: option, callback_data: `generator:answer:${question.id}:${index}` },
        ]),
        ...
        ...question.options.map((option, index) => [
          {
            ...
            callback_data: `generator:multi:${question.id}:${index}`,
          },
        ]),
  ...
  if (question.type === "rating") {
    ...
    for (let value = settings.min; value <= settings.max; value += settings.step) {
      ...
      if (!row || row.length >= 5) values.push([button]);
  ...
  if (question.type === "boolean")
    return {
      inline_keyboard: [
        [
          { text: "✅ 是", callback_data: `generator:answer:${question.id}:true` },
          { text: "❌ 否", callback_data: `generator:answer:${question.id}:false` },
        ],
        ...questionKeyboard(question.required).inline_keyboard,
      ],
    };
...
function imageKeyboard(question: GeneratorQuestion, uploaded: number): InlineKeyboardMarkup {
  const settings = questionSettings(question);
  const rows: InlineKeyboardMarkup["inline_keyboard"] = [];
  if (uploaded >= settings.minImages)
    rows.push([{ text: "完成上传", callback_data: `generator:image_done:${question.id}` }]);
  rows.push(...questionKeyboard(question.required).inline_keyboard);
  return { inline_keyboard: rows };
}

async function renderParticipant(ctx: BotContext, userId: number, session: Session): Promise<void> {
  ...
  if (session.index < questions.length) {
    ...
    const controls =
      question.type === "image"
        ...
        : (answerKeyboard(
          ...
          ) ?? questionKeyboard(question.required));
    await renderParticipantScreen(
      ...
      [
        ...
        question.prompt,
        question.type === "image"
          ? `请发送图片（${uploaded}/${questionSettings(question).maxImages}）。`
...
export async function handleImageGeneratorAdminMessage(
  ...
  if (current.kind === "import") {
    ...
    if (!document?.file_id || (document.file_size ?? 0) > 512 * 1024) {
      current.messageId = await render(
        ctx,
        current.chatId,
        userId,
        current.messageId,
        "REPORT_GENERATOR_IMPORT",
        "请发送不超过 512 KB 的 JSON 文件，或发送 /cancel。",
        { inline_keyboard: [] },
      );
  ...
  if (current.kind === "background" || current.kind === "report_background") {
    if (!message.photo?.length) {
      current.messageId = await render(
        ...
        userId,
        current.messageId,
        "IMAGE_GENERATOR_BACKGROUND_UPLOAD",
        current.kind === "report_background"
          ? "请发送报告默认背景图片，或发送 /cancel。\n\n系统会自动加入遮罩和内容卡，保护文字可读性。"
          : "请发送预设背景图片，或发送 /cancel。",
        { inline_keyboard: [] },
      );
      await putState(ctx, adminKey(userId), current);
      return true;
  ...
  const [variableName, prompt, typeRaw, requiredRaw, optionsRaw, settingsRaw] = text
    .split("|")
    .map((item) => item.trim());
  const allowedTypes: GeneratorQuestionType[] = [
    "text",
    "long_text",
    "number",
    "single",
  ...
  for (const part of (settingsRaw ?? "").split(",")) {
    ...
    if (["minImages", "maxImages", "min", "max", "step"].includes(key ?? "") && Number.isFinite(value))
      settings[key as keyof GeneratorQuestionSettings] = value;
  }
  if (
    !variableName ||
    !prompt ||
    !type ||
    !/^[A-Za-z][A-Za-z0-9_-]{0,40}$/.test(variableName) ||
    (["single", "multiple"].includes(type) && options.length < 2) ||
    (type === "image" &&
      settings.maxImages !== undefined &&
      (!Number.isInteger(settings.maxImages) || settings.maxImages < 1 || settings.maxImages > 10))
  ) {
    current.messageId = await render(
      ctx,
      current.chatId,
  ...
  if (session.index < questions.length) {
    ...
    if (["single", "multiple", "rating", "boolean"].includes(question.type)) {
      await renderParticipantScreen(
        ...
        "IMAGE_GENERATOR_STEP",
        "请使用下方按钮回答此题。",
        answerKeyboard(question) ?? questionKeyboard(question.required),
      );
      return true;
    }
    if (question.type === "image") {
      ...
      if (images.length >= settings.maxImages) {
        await renderParticipantScreen(
          ...
          session,
          "IMAGE_GENERATOR_STEP",
          `最多只能上传 ${settings.maxImages} 张图片。`,
          imageKeyboard(question, images.length),
        );
        return true;
      }
      ...
      if (question.type === "number" || question.type === "rating") {
        ...
        if (question.type === "rating" && (value < settings.min || value > settings.max)) {
          await renderParticipantScreen(
            ...
            session,
            "IMAGE_GENERATOR_STEP",
            `评分范围为 ${settings.min}-${settings.max}。`,
            answerKeyboard(question) ?? questionKeyboard(question.required),
          );
          return true;
        }
...
async function validateGeneratorForPublish(ctx: BotContext, generatorId: number): Promise<void> {
  ...
  for (const question of questions) {
    if (!/^[A-Za-z][A-Za-z0-9_-]{0,40}$/.test(question.variableName) || names.has(question.variableName))
      throw new Error(`问题变量 ${question.variableName} 无效或重复`);
    names.add(question.variableName);
    if ((question.type === "single" || question.type === "multiple") && question.options.length < 2)
      throw new Error(`${question.variableName} 至少需要两个选项`);
    if (question.type === "image") {
      const settings = questionSettings(question);
  ...
  for (const variable of definition.variables) {
    ...
    if (match[1] === "images" && question.type !== "image")
      throw new Error(`模板图片变量 {{${match[2]}}} 必须关联 image 类型问题`);
    if (match[1] === "fields" && question.type === "image")
      throw new Error(`模板文字变量 {{${match[2]}}} 不能关联 image 类型问题`);
  }
}
...
export async function handleImageGeneratorCallback(
  ...
  if (isAdmin) {
    ...
    if (data.startsWith("generator:new:")) {
      ...
      const state: AdminState = {
        kind: "new",
        templateId: Number(data.slice("generator:new:".length)),
        chatId,
        messageId,
      };
      state.messageId = await render(
    ...
    if (data.startsWith("generator:style:")) {
      ...
      if (
        !Number.isSafeInteger(generatorId) ||
        !template ||
        template.status !== "published" ||
        !template.currentVersion
      )
        throw new Error("报告样式不存在");
    ...
    if (data.startsWith("generator:template:")) {
      ...
      if (templateId !== null) {
        const template = await getVisualTemplateById(ctx.db, templateId);
        if (!template || template.status !== "published" || !template.currentVersion)
          throw new Error("请选择已发布的报告模板");
        await updateGenerator(ctx.db, generatorId, { templateId });
        await showAdminDetail(ctx, chatId, userId, generatorId, messageId);
        await answerCallbackQuery(ctx.botToken, callback.id, "报告模板已更新");
        return true;
      }
      const templates = (await listVisualTemplates(ctx.db)).filter(
        (template) => template.status === "published" && template.currentVersion,
      );
      await render(
        ctx,
        chatId,
        ...
        "选择一个已发布的 Visual Report Template：",
        {
          inline_keyboard: [
            ...templates.map((template) => [
              {
                text: `${template.type === "report" ? "📊 " : "🎨 "}${template.name}`,
                callback_data: `generator:template:${generatorId}:${template.id}`,
              },
            ]),
    ...
    if (data.startsWith("generator:report_background:")) {
      ...
      state.messageId = await render(
        ...
        userId,
        messageId,
        "REPORT_GENERATOR_DEFAULT_BACKGROUND",
    ...
    if (data.startsWith("generator:delete:")) {
      ...
      const generatorId = Number(generatorRaw);
      const questionId = Number(questionRaw);
      if (Number.isSafeInteger(generatorId) && Number.isSafeInteger(questionId))
        await deleteGeneratorQuestion(ctx.db, questionId);
      await showAdminDetail(ctx, chatId, userId, generatorId, messageId);
    ...
    if (data.startsWith("generator:move:")) {
      ...
      if (
        ...
        await moveGeneratorQuestion(ctx.db, generatorId, questionId, direction);
      await showAdminDetail(ctx, chatId, userId, generatorId, messageId);
      await answerCallbackQuery(ctx.botToken, callback.id);
      return true;
    }
    if (data.startsWith("generator:mode_menu:") || data.startsWith("generator:mode_set:")) {
      ...
      const resolvedGeneratorId = data.startsWith("generator:mode_set:") ? Number(data.split(":")[2]) : generatorId;
    ...
    if (data.startsWith("generator:contrast_menu:")) {
      ...
      await render(
        ...
        userId,
        messageId,
        "REPORT_GENERATOR_CONTRAST",
        "🌓 内容颜色\n\n自动：照片背景上使用浅色内容卡；深色背景可选白字深色卡。",
        {
          inline_keyboard: [
    ...
    if (data.startsWith("generator:contrast_set:")) {
      ...
      if (
        !Number.isSafeInteger(generatorId) ||
        !reportContrastMode ||
        !["auto", "light", "dark"].includes(reportContrastMode)
      )
        return false;
      await updateGenerator(ctx.db, generatorId, {
        reportContrastMode: reportContrastMode as "auto" | "light" | "dark",
      });
      await showAdminDetail(ctx, chatId, userId, generatorId, messageId);
    ...
    if (data.startsWith("generator:delete_ask:")) {
      ...
      await render(
        ...
        userId,
        messageId,
        "REPORT_GENERATOR_DELETE_CONFIRM",
        `确认永久删除报告“${generator.name}”？\n\n题目、填写结果和排队任务都会删除。`,
        {
          inline_keyboard: [
            [
  ...
  if (data === "generator:list_user") {
    ...
    await render(
      ...
      userId,
      messageId,
      "REPORT_GENERATOR_RETIRED",
      "📊 报告需要在问卷提交完成后生成。\n\n请先从问卷列表选择一份问卷并完成填写。",
      {
        inline_keyboard: [
          [{ text: "📝 浏览问卷", callback_data: "home:surveys" }],
          [{ text: "⬅️ 返回首页", callback_data: "home:menu" }],
        ],
      },
    );
    await answerCallbackQuery(ctx.botToken, callback.id);
  ...
  if (data === "generator:cancel") {
    ...
    await render(ctx, chatId, userId, messageId, "IMAGE_GENERATOR_CANCEL", "已取消报告生成。", {
      inline_keyboard: [[{ text: "📊 报告生成", callback_data: "generator:list_user" }]],
    });
    await answerCallbackQuery(ctx.botToken, callback.id);
    return true;
  }
  ...
  if (data.startsWith("generator:multi:")) {
    ...
    const selected = Array.isArray(currentValue)
      ...
      : [];
    session.values[question.variableName] = selected.includes(option)
  ...
  if (data.startsWith("generator:multi_done:")) {
    ...
    if (question.required && selected.length === 0) {
      await answerCallbackQuery(ctx.botToken, callback.id, "请至少选择一项");
      return true;
    }
    session.index += 1;
    await putState(ctx, sessionKey(userId), session);
  ...
  if (data.startsWith("generator:image_done:")) {
    ...
    const value = session.values[question.variableName];
    const count = Array.isArray(value) ? value.length : value ? 1 : 0;
    if (count < questionSettings(question).minImages) {
      await answerCallbackQuery(ctx.botToken, callback.id, "图片数量不足");
      return true;
    }
    session.index += 1;
    await putState(ctx, sessionKey(userId), session);
  ...
  if (data === "generator:render") {
    ...
    await render(
      ...
      userId,
      messageId,
      "IMAGE_GENERATOR_QUEUED",
      "🎨 正在生成你的报告，完成后会直接发送 PNG。",
      { inline_keyboard: [] },
    );
    await ctx.cache?.delete(sessionKey(userId));
