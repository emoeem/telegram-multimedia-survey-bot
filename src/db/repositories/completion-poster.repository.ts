...
export async function getCompletionPosterSetting(db: D1Database, surveyId: number): Promise<CompletionPosterSetting> {
  const row = await db
    .prepare("SELECT * FROM survey_completion_posters WHERE survey_id = ? LIMIT 1")
    .bind(surveyId)
    .first<{ survey_id: number; enabled: number; style: string; updated_at: string }>();
...
export async function saveCompletionPosterSetting(
  ...
  await db
    .prepare(
      `INSERT INTO survey_completion_posters (survey_id, enabled, style, updated_at)
    VALUES (?, ?, ?, ?)
    ON CONFLICT(survey_id) DO UPDATE SET enabled = excluded.enabled, style = excluded.style, updated_at = excluded.updated_at`,
    )
    .bind(input.surveyId, input.enabled ? 1 : 0, input.style, timestamp)
    .run();
