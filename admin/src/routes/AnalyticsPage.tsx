    ...
    <div>
      <section className="card">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-2">
          ...
          <div className="rounded-xl border border-[var(--color-edge)] p-3">
            ...
            {statusDonut ? (
              <EChart option={statusDonut} style={{ height: 180 }} opts={{ renderer: "svg" }} />
            ) : (
      ...
      {timeline ? (
        <section className="mt-5 card">
          ...
          <EChart option={timeline} style={{ height: 200, width: "100%" }} opts={{ renderer: "svg" }} />
        </section>
      ) : null}
              ...
              return (
                <div key={first.questionId} className="rounded-xl border border-[var(--color-edge)] p-4">
                  <div className="mb-3 flex items-center gap-3">
                    <span className="inline-flex items-center rounded-md bg-gradient-to-br from-[var(--color-primary)] to-[#7c3aed] px-2 py-0.5 text-xs font-bold text-white font-mono">
                      Q{first.questionId}
                    </span>
                    <h3 className="flex-1 font-medium">{first.questionTitle}</h3>
                    {total > 0 ? (
                      <span className="rounded-full border surface-warning-soft px-2 py-0.5 text-xs font-medium text-warning-strong">
                      ...
                      {group.map((item, idx) => (
                        <div key={item.optionId}>
                          <div className="flex justify-between gap-3 text-sm">
                            <span
                              className={idx === championIdx && total > 0 ? "font-semibold text-warning-strong" : ""}
                            >
                              {item.optionLabel}
                            </span>
          ...
          <div className="grid gap-5 md:grid-cols-[1fr_1fr] items-start">
            {numericBar ? (
              <div className="rounded-xl border border-[var(--color-edge)] p-2">
                <EChart
                  option={numericBar}
                  style={{ height: Math.max(200, data.numericStats.length * 42), width: "100%" }}
                  opts={{ renderer: "svg" }}
                />
              </div>
            ) : null}
              ...
              {data.numericStats.map((item) => (
                <article key={item.questionId} className="rounded-xl border border-[var(--color-edge)] p-4">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 text-sm text-[var(--text-soft)]">
                    <span>
                      样本：<strong>{item.count}</strong>
                    </span>
                    <span>
                      平均值：<strong className="text-[var(--color-primary)]">{item.average ?? "—"}</strong>
                    </span>
